...
```{r Read in Counts and RPKM, eval=TRUE, echo=TRUE}
...
For CRAN packages, for example ```ggplot2```, installation can be done with the following code:
...
```{r examine data, echo=TRUE, eval = TRUE}
...
Here we will create some exploratory plots to visualize the major axes of variation in our data using Principal Components Analysis.  We'll check for association of the top components with known variables in our metadata to see if these can explain some of the variation.
...
# Hour II: Normalization and Quality Control of scRNA-seq
Apua's overview of QC metrics and assessment of normalization metrics using the R package scone. To view this material, visit his GitHub page [here](https://github.com/apuapaquola/fog).  

## Normalization
In the previous section we used data that have been normalized by RPKM.  It is vital that the RNA-seq counts be normalized to ensure that differences found are not due to things like sequencing depth, library preparation, or batch effects.  Since scRNA-seq data exhibits much more technical variablity and much lower detection rate than bulk RNA-seq, new specialized normalization methods have been developed.  Here we wil use the pool & deconvolve normalization method of Lun et al. (2016) to adjust for library size.

But first, for ease in downstream analysis with various R packages we'll make use of today, we'll convert the data.frames that currently (separately) house the counts and cell metadata into a Bioconductor object called an SCESet introduced by the ```scater``` package.  This object is a container that can hold raw and normalized expression values, along with the metadata for both samples and genes, in one place. 
...
Now that our SCESet has been created, we'll perform some basic preprocessing and normalization here to adjust for library size using the pool and deconvolve method in the ```scran``` package, as well as remove genes with very low expression.
...
One aspect of variation among single cell data is the proportion of genes detected in each cell.  It is thought that this is determined by a combination of biological (gene is off or on) and technical (gene is present but not at a level high enough to be detected by the assay) factors. Teasing this out is still an active and ongoing area of research.  Here we'll take a look at the distribution of the detection rate (proportion of genes detected) across our population of cells, and see how that correlates with the principal components we computed above.
...
There does seem to be an association between PC1 and the detection rate.  Recall that some of this variability will be biological and some will be technical.  Note that some analysts will adjust for this effect in downstream analyses, with the assumption that it is largely technical.  This is also an active area of research.

# Hour III: Analysis Modules

## Module 1: Identify the highly variable genes
...
Here we will perform some calculations to help us identify genes that have high variability.  To do so, we have to take into account the relationship between mean expression level and variance of expression level.  Namely, that what we observe in RNA-seq count data is that genes with higher expression have higher variance.  Note that for data on the log-scale, the trend is reverse (variance of log-expression decreases for higher mean log-expression genes).  As an aside, note that this is an expected property of count distributions, but often biological data exhibit even more variation than expected (this is commonly referred to as overdispersion in the literature).  This complex relationship between the mean and variance can be explained with the Negative Binomial distribution, which is often used to model count data from RNA-seq experiments.  In this module we are not going to assume any parametric distribution to relate the mean and variance, but instead fit a smooth curve to the mean and variance relationship we observe in our dataset.
...
## Module 2: Identify Differentially Expressed genes
...
To get more insight into the models fit by SCDE, we can use the related ```scde.test.expression.difference``` function to visualize the results for a particular gene. For example, we can view the cell-specific posterior distributions for the two different neuronal subtypes for the Differentially Expressed gene *Gad1*: 
...
In contrast, we can look at another gene that is not differentially expressed (*Rgs17*) and see that the cell-specific posterior distributions do not look globally different between excitatory and inhibitory neurons. 
...
Let's also look at the SCDE results for the *Vip* gene, which was one of the top 25 highly variable genes and showed an interesting subgrouping pattern in the boxplots and heatmap above.  This gene was not one of the significant DE genes found by SCDE, but there does seem to be a difference in the expression pattern between the neural subtypes.

```{r scde plot Vip, eval=TRUE, echo=TRUE}
...
```{r heatmap scde, eval=TRUE, echo=TRUE}
# extract matrix of hvg expression for plotting
...
First, we construct a ```SummarizedExperiment``` object that contains our count data and cell metadata for input to ```scDD```.  Note that this is very similar to the ```SCESet``` object class used by ```scater```; that class is based on the ```ExpressionSet``` class.  We'll reuse the ```group``` and ```which.neur``` neuronal subtype vector since they already contain the subset cells we wish to analyze.  Note that scDD expects normalized counts (not log-transformed), so we extract them from the ```eset.hvg``` object using the ```exprs``` function.  We'll also create a list of the hyperparameters to use in model-fitting.

```{r scdd setup, eval = TRUE, echo = TRUE}
library(scDD)
library(Biobase)
...
```{r scdd fit, eval=TRUE, echo=TRUE}
# find DE genes between excitatory and inhibitory neuronal subtypes
...
```{r scdd gad1, eval=TRUE, echo=TRUE}
...
The *Rgs17* gene was not differentially distributed.

```{r scdd Rgs17, eval=TRUE, echo=TRUE}
...
```{r scdd Pla2g7, eval=TRUE, echo=TRUE}
...
```{r scdd Scg2, eval=TRUE, echo=TRUE}
...
```{r heatmap DM, eval=TRUE, echo=TRUE}
# extract matrix of hvg expression for plotting
...
## Module 3: Order cells by "Psuedotime" (temporal-spatial variation): Monocle

Just like we saw in the previous two modules, it can be convenient to store all information about an experiment (measurements *and* metadata) in one R object.  This is a recurring theme in the Bioconductor project, so many R packages on Bioconductor rely on constructs like these.  The benefits are gains in efficiency and reduction of errors in associating metadata objects back to the measurement objects, and often many package developers use objects that are compatible across several methods.  However, with the explosion in methods development for scRNA-seq analysis, we have not yet established a 'standard' object type for storing and analysing scRNA-seq data in R.  The result is that packages developed simulataneously and independently have slightly different formatting requirements (as we have seen in the packages we've used today).  So long story short, we have to convert our ```SCESet``` object from the ```scater``` package to a similar ```CellDataSet``` object for use with the ```monocle``` package.  Don't worry about the details here.

Our goal in this analysis module is to discover a latent trajectory of variation using ```Monocle``` which may represent spatial organization of the cells.  Since it is recommended to use genes that are believed to be important in determining where cells are located in relation to eachother (the 'ordering'), we use the dataset that only contains the top 1000 highly variable genes that we constructed a previous analysis module.  
...
The spanning trees above suggest that the Cre reporter line explains more of the variation among the Inhibitory cells than does the spatial information (lower versus upper layer).  Is the same true for the Exitatory cells?

How does the spanning tree change if we allow a different number of paths?  How does the spanning tree change if we include different gene sets other than the top 100 highly variable genes?  
