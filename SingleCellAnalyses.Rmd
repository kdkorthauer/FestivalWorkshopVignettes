...
rpkm.raw <- read.csv("genes_rpkm.csv", stringsAsFactors = FALSE, header=TRUE, row.names = 1)
str(rpkm.raw[,1:20]) # restrict to the first 20 columns (cells) 
rpkm <- log(rpkm + 1, 2)
```
...
rm(gene.var, rpkm.pca, rpkm.top1000, df); gc()  # clean up workspace
```
...
rm(detectionRate); gc()  #clean up workspace
```
